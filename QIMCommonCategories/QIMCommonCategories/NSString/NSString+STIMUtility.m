...
@implementation NSString (STIMUtility)
...
- (NSDictionary *)stimDB_dictionaryFromQueryComponents
{
    ...
    for(NSString *keyValuePairString in [self componentsSeparatedByString:@"&"])
    {
        ...
        NSString *key = [[keyValuePairArray objectAtIndex:0] stimDB_stringByDecodingURLFormat];
        NSString *value = [[keyValuePairArray objectAtIndex:1] stimDB_stringByDecodingURLFormat];
...
- (NSDictionary *)stimDB_dictionaryFromParamComponents
{
    ...
    for(NSString *keyValuePairString in [self componentsSeparatedByString:@"&"])
    {
        ...
        if(pairCount == 2)
        {
            key = [[keyValuePairArray objectAtIndex:0] stimDB_stringByDecodingURLFormat];
            value = [[keyValuePairArray objectAtIndex:1] stimDB_stringByDecodingURLFormat];
        }
        else if(pairCount == 1)
        {
            key = [[keyValuePairArray objectAtIndex:0] stimDB_stringByDecodingURLFormat];
        ...
        {
            //TODO: no such method
            [paramComponents setSTIMSafeObject:value forKey:key];
        }
    }
...
- (NSString *)stimDB_getYYMMDDFWW
{
    ...
    NSString *defaultDateText  = [NSString stringWithFormat:@"%@ %@", self, [STIMUtility getFullWeekend:weekday]];
...
{
    if (![data stimDB_isStringSafe] || ![salt stimDB_isStringSafe])
    {
        return nil;
    }
