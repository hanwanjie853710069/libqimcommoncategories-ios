...
@implementation UIImage (STIMUtility)
...
//截取全屏
+ (UIImage *)stimDB_screenShotInWindow {
...
{
	CGSize size = [self stimDB_fitSize:sideLenght];
...
- (CGSize)stimDB_fitSize:(CGFloat)sideLenght
{
	CGFloat scale;
	CGSize newsize;
...
+(CGSize)stimDB_getImageSizeWithURL:(id)imageURL
{
    ...
    if([pathExtendsion isEqualToString:@"png"]){
        size =  [self stimDB_getPNGImageSizeWithRequest:request];
    }
    else if([pathExtendsion isEqual:@"gif"])
    {
        size =  [self stimDB_getGIFImageSizeWithRequest:request];
    }
    else{
        size = [self stimDB_getJPGImageSizeWithRequest:request];
    }
...
//  获取PNG图片的大小
+(CGSize)stimDB_getPNGImageSizeWithRequest:(NSMutableURLRequest*)request
{
...
//  获取gif图片的大小
+(CGSize)stimDB_getGIFImageSizeWithRequest:(NSMutableURLRequest*)request
{
...
//  获取jpg图片的大小
+(CGSize)stimDB_getJPGImageSizeWithRequest:(NSMutableURLRequest*)request
{
...
+ (UIImage *)stimDB_imageNamed:(NSString *)name {
    ...
    NSString * path = [[NSBundle mainBundle] pathForResource:@"STIMSDK" ofType:@"bundle"];
...
+ (NSString *) stimDB_image2DataURL: (UIImage *) image
{
    ...
    if ([self stimDB_imageHasAlpha: image]) {
